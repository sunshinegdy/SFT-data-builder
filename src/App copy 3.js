...
function App() {
  ...
  const generateAIResponse = async (content) => {
    try {
      setIsLoading(true);
      console.log('发送到 AI 的内容:', content);
...
示例JSON输出：
{
  ...
  "history": []
      ...
      console.log('API 请求内容:', requestBody);
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        ...
        console.log('AI 返回的 JSON 数据:', aiResponse);
        
        // 解析并格式化输出 JSON 数据
        const parsedResponse = JSON.parse(aiResponse);
        console.log('解析后的 JSON 数据:', JSON.stringify(parsedResponse, null, 2));
